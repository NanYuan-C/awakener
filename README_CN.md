...
**人设决定一切。** Awakener 本身是中性的运行时——agent 成为什么样的存在，完全取决于你配置的两个提示词文件：

| 人设风格 | Agent 的行为 |
|---|---|
| 自由探索者 | 自由漫游，做任何感兴趣的事，没有指定任务 |
| 半工半玩 | 维护一个项目，完成后用剩余时间自由探索 |
| 专职开发者 | 端到端开发一个产品——写代码、推送 GitHub、维护演示站 |
| 运维 Agent | 7×24 监控服务、修复问题、保持系统稳定运行 |
| 研究员 | 阅读资料、整理笔记、每天输出指定方向的研究报告 |
| 内容运营 | 抓取数据、整理内容、持续更新网站或知识库 |
| 个人助理 | 处理你的任务收件箱，执行需求，完成后汇报结果 |

换一套人设，装上合适的 Skills，就是一个完全不同的 agent——不需要改任何代码。运行时不变，你的提示词和 Skills 定义产品。
